...
ords.create_service(
      ...
      p_pattern =>  'vendita/' ,
      p_items_per_page => 25,
      p_source  =>  'select * from d10_vendita order by id');
commit;
end;
/
/*
...
begin
  ords.create_role('d10.ROLE');     
 
  ords.create_privilege(
      p_name => 'd10.PRIVILEGE',
      p_role_name => 'd10.ROLE',
      p_label => 'd10 data',
      p_description => 'Provide access to d10 data');
  commit;
end;
/

--select id,name from user_ords_privileges where name = 'd10';

begin
 ords.create_privilege_mapping(
      p_privilege_name => 'd10.PRIVILEGE',
      p_pattern => '/d10/*');     
  commit;
end;
/
...
begin 
 oauth.create_client(
      ...
      p_privilege_names => 'd10.PRIVILEGE',
      p_support_email =>'pippo@pippo.com');
 commit;
end;
/

--select name, client_id,client_secret from user_ords_clients where name = 'client_d10';
...
--select * from user_ords_client_roles where client_name = 'client_d10';
https://cnde8nae4maapcd-capancioni.adb.eu-zurich-1.oraclecloudapps.com/ords/cy0/d10/vendita/
https://cnde8nae4maapcd-capancioni.adb.eu-zurich-1.oraclecloudapps.com/ords/cy0/oauth/token
...
BEGIN
    apex_web_service.oauth_authenticate_credential(
        p_token_url => 'https://cnde8nae4maapcd-capancioni.adb.eu-zurich-1.oraclecloudapps.com/ords/cy0/oauth/token',
        p_credential_static_id => 'client_d10');
...
apex_web_service.oauth_authenticate(
        p_token_url => 'https://cnde8nae4maapcd-capancioni.adb.eu-zurich-1.oraclecloudapps.com/ords/cy0/oauth/token',
        p_client_id     => 'neink6cNIZhUJkT1LAb26g..',
        p_client_secret => 'DUgLUpTkhFwb5xI7mqVfYw..');
        l_response := APEX_WEB_SERVICE.MAKE_REST_REQUEST(p_url => 'https://cnde8nae4maapcd-capancioni.adb.eu-zurich-1.oraclecloudapps.com/ords/cy0/d10/vendita/',
                                                         p_http_method => 'GET',
                                                         p_scheme => 'OAUTH_CLIENT_CRED');    
       htp.prn(l_response);
