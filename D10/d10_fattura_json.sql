...
select json_object(
  ...
  from d10_fattura_testata t
  join d10_cliente c on t.cliente_id = c.id;

select * 
  from d10_fattura_dettaglio;
...
create or replace view d10_fattura_json_vw as
select t.id,
       json_object(
                   t.id,
                   t.cliente_id,
...
select * from d10_fattura_json_vw;

select * from json_table(
                    (
                    select fattura 
                      from d10_fattura_json_vw
                     where id =1
                    ),
                    '$[*]'
                        columns(
